...
<?php else: ?>
    ...
    <div class="card">
        ...
        <form method="post" action="<?= site_url('mahasiswa/tim/gps') ?>" id="form-gps" class="location-picker-form">
                <?= csrf_field() ?>
                <p class="location-picker-note">Klik titik lokasi penelitian di peta, atau gunakan lokasi perangkat saat berada di lapangan. Setelah memilih titik, alamat lengkap akan terdeteksi otomatis.</p>
                <div class="location-picker">
                    ...
                    <input type="hidden" name="latitude" id="input-lat" data-location-latitude value="<?= esc($k['latitude'] ?? '') ?>" required>
                    <input type="hidden" name="longitude" id="input-lng" data-location-longitude value="<?= esc($k['longitude'] ?? '') ?>" required>
                </div>

                <div class="field" style="margin-top:1rem;">
                    <label for="alamat-penelitian" style="font-weight:600;">
                        Alamat penelitian
                        <span id="alamat-loading" style="display:none;font-size:.75rem;color:#6366f1;margin-left:.5rem;">&#9203; Mendeteksi alamat&hellip;</span>
                        <span id="alamat-ok" style="display:none;font-size:.75rem;color:#22c55e;margin-left:.5rem;">&#10003; Alamat terdeteksi</span>
                    </label>
                    <input type="text" id="alamat-penelitian" name="alamat_penelitian"
                           value="<?= esc($k['alamat_penelitian'] ?? '') ?>"
                           placeholder="Alamat terisi otomatis setelah pilih titik di peta, atau ketik manual"
                           style="width:100%;">
                    <span class="field-hint">Anda dapat mengedit alamat ini secara manual bila perlu.</span>
                </div>
        ...
        (function () {
            ...
            function reverseGeocode(lat, lng) {
                var loadEl = document.getElementById('alamat-loading');
                var okEl   = document.getElementById('alamat-ok');
                var input  = document.getElementById('alamat-penelitian');
                if (!input) return;
            ...
            // Polling sederhana: cek setiap 500ms apakah hidden input berubah
            var prevLat = '', prevLng = '';
            setInterval(function(){
                var latEl = document.getElementById('input-lat');
                var lngEl = document.getElementById('input-lng');
                if (!latEl || !lngEl) return;
                var lat = latEl.value, lng = lngEl.value;
                if ((lat !== '' || lng !== '') && (lat !== prevLat || lng !== prevLng)) {
                    prevLat = lat; prevLng = lng;
                    if (lat && lng) reverseGeocode(parseFloat(lat), parseFloat(lng));
                }
            }, 500);

            // Tombol "Gunakan lokasi saya": setelah 2 detik coba geocode
            var btnUse = document.querySelector('[data-location-use]');
            if (btnUse) {
                btnUse.addEventListener('click', function(){ setTimeout(tryGeocode, 2000); });
            }
        })();
        </script>
        <?php elseif ($hasGps): ?>
